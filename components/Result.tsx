...
const Result: React.FC<ResultProps> = ({ resultData, rawScores, onRetest, onOpenConsultant, onViewArchive, isArchiveMode = false, user, onLogin, onLogout }) => {
  ...
  const identityLabel = (t as any).assertive_label && (t as any).turbulent_label
    ? ((percentages.A >= percentages.Turbulent) ? (t as any).assertive_label : (t as any).turbulent_label)
    : ((percentages.A >= percentages.Turbulent) ? '堅定型' : '動盪型');
  const displayKeywords = (extraI18n as any)?.keywords || resultData.keywords;

  const dimensionData = [
    { key: 'E/I', label: (t as any).dim_ei_label || '能量流向 (ENERGY)', text: (t as any).dim_ei_text || '' },
    { key: 'S/N', label: (t as any).dim_sn_label || '感知模式 (INFORMATION)', text: (t as any).dim_sn_text || '' },
    { key: 'T/F', label: (t as any).dim_tf_label || '判斷準則 (DECISIONS)', text: (t as any).dim_tf_text || '' },
    { key: 'J/P', label: (t as any).dim_jp_label || '生活態度 (LIFESTYLE)', text: (t as any).dim_jp_text || '' },
    { key: 'A/T', label: (t as any).dim_at_label || '自我認同 (IDENTITY)', text: (t as any).dim_at_text || '' },
  ];
  ...
  // identityChinese is kept for backward compat but we now prefer identityLabel from translations
            ...
            <div className="max-w-4xl mx-auto px-6 text-center">
              ...
              <div className="mt-8 md:mt-10 max-w-md mx-auto text-center transform transition-transform hover:scale-[1.02]">
                <a
                  ...
                  className="inline-block w-full py-4 bg-kiwi-dark text-white rounded-lg font-bold tracking-[0.2em] uppercase hover:bg-gray-800 transition-all active:scale-95 shadow-lg"
                  onClick={() => trackAction('click_passport_auto_unlock', { mbtiType: resultData.id, variant: identitySuffix })}
                >
                  {(t.passport_cta || '').replace('{type}', `${resultData.id}-${identitySuffix}`)}
                </a>
              </div>
            </div>
          </div>
              ...
              <div className="md:col-span-5">
                ...
                <div className="mt-6 md:mt-8 pt-6 border-t border-gray-100 flex justify-between items-end">
                  <div className="space-y-1 text-left">
                    ...
                    <p className="text-xs md:text-sm font-display font-bold text-kiwi-dark tracking-widest italic uppercase">{identityLabel} ({identitySuffix})</p>
                  </div>
                ...
                <div className="py-16 md:py-24 border-b border-gray-100 bg-gradient-to-br from-purple-50/50 to-pink-50/50">
                  <div className="max-w-3xl mx-auto px-6 text-center">
                    ...
                    <h3 className="text-3xl md:text-4xl font-serif font-bold text-gray-800 mb-6">
                      {resultData.name}的小彩蛋
                    </h3>
                    <p className="text-lg md:text-xl text-gray-700 leading-relaxed mb-8 px-4">
                      去月島搜尋 kiwimu，解鎖專屬於你的隱藏彩蛋...
                    </p>
                    <a
